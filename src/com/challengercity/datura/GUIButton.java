...
public class GUIButton extends GUI {
    ...
    public void checkMouse() {
        if (Mouse.getX()>posX&&Mouse.getX()<(posX+width) && (Datura.screenHeight-Mouse.getY())>posY&&(Datura.screenHeight-Mouse.getY())<(posY+height) && Datura.guiCooldown<=0) {
            hovered = 1;
            if (Mouse.isButtonDown(0)) {
                Datura.currentScreen.actionPerformed(actionId);
                Datura.guiCooldown=60;
            }
        } else {
            hovered = 0;
        }
