...
public class ResourcePool {
    ...
    public static Texture getTexture(String imageName, String fileExt) { // Tiles.png
        if (!loadedTextures.containsKey(imageName+fileExt)) {
            try {
                loadedTextures.put(imageName+fileExt, TextureLoader.getTexture(fileExt, ClassLoader.getSystemClassLoader().getResourceAsStream("com/challengercity/datura/resources/"+imageName+(fileExt.toLowerCase()))));
            } catch (Exception ex) {
                Datura.log(ResourcePool.class, "Could not load texture - "+imageName+fileExt);
                Display.destroy();
                System.exit(1);
            }
        }
    ...
    public static TrueTypeFont getFont(String fontName, int fontSize) { // Courier_12.ttf
        if (!loadedFonts.containsKey(fontName+"_"+fontSize+".ttf")) {
            try {
                ...
                Datura.log(ResourcePool.class, "Could not load font - "+fontName+".ttf");
                Display.destroy();
                System.exit(1);
            }
        }
    ...
    public static void loadFrequentResources() {
        if (!loadedTextures.containsKey("gui_button.png")) {
            try {
                loadedTextures.put("gui_button.png", TextureLoader.getTexture(".png", ClassLoader.getSystemClassLoader().getResourceAsStream("com/challengercity/datura/resources/gui_button.png")));
            } catch (Exception ex) {
                Datura.log(ResourcePool.class, "Could not load texture - gui_button.png");
            }
        }
        if (!loadedFonts.containsKey("Courier_14.ttf")) {
            try {
                ...
                Datura.log(ResourcePool.class, "Could not load font - Courier_14.ttf");
            }
        }
    }
    
//    public static Texture getRemoteTexture(String imageName, String fileExt) { // TODO Enable image downloading for skins
...
//        } catch (Exception ex) {
//            ex.printStackTrace();
//            Underground.log(ResourceLoader.class, "Could not load texture - "+imageName+fileExt);
//        }
//     return texture;
//    }
}
